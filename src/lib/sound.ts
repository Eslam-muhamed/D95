...
/**
 * Plays an authentic, cute Astro Bot robotic chirp & cheerful squeak!
 * High-speed electronic pitch slide followed by a sparkling robotic high-harmonic chime.
 */
export function playAstroBotChirp(): void {
    try {
        const ctx = getAudioContext();
        const now = ctx.currentTime;
        ...
        // Chirp 1: Rapid enthusiastic rising squeak (880Hz -> 2100Hz)
        const osc1 = ctx.createOscillator();
        const gain1 = ctx.createGain();
        osc1.type = 'sine';
        osc1.frequency.setValueAtTime(880, now);
        osc1.frequency.exponentialRampToValueAtTime(2200, now + 0.07);
        ...
        // Chirp 2: Happy secondary bounce (+0.08s: 1900Hz -> 2800Hz)
        const t2 = now + 0.075;
        const osc2 = ctx.createOscillator();
        const gain2 = ctx.createGain();
        osc2.type = 'triangle';
        osc2.frequency.setValueAtTime(1760, t2);
        osc2.frequency.exponentialRampToValueAtTime(2950, t2 + 0.08);

        gain2.gain.setValueAtTime(0, t2);
        gain2.gain.linearRampToValueAtTime(0.35, t2 + 0.004);
        ...
        // Sparkle overtone (Astro's antenna light twinkle)
        const t3 = now + 0.12;
        const osc3 = ctx.createOscillator();
        const gain3 = ctx.createGain();
        osc3.type = 'sine';
        osc3.frequency.setValueAtTime(3520, t3);
        osc3.frequency.exponentialRampToValueAtTime(3800, t3 + 0.1);

        gain3.gain.setValueAtTime(0, t3);
        gain3.gain.linearRampToValueAtTime(0.2, t3 + 0.005);
        gain3.gain.exponentialRampToValueAtTime(0.0001, t3 + 0.18);
