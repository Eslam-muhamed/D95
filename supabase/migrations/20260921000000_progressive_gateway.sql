-- Migration: 20260921000000_progressive_gateway.sql
-- Description: Progressive Risk-Based Gateway for Bookings

-- 1. Add client_ip to ps_bookings to track the source of requests
ALTER TABLE public.ps_bookings ADD COLUMN IF NOT EXISTS client_ip INET;

-- 2. Update create_booking_atomic to record client_ip
...
BEGIN
  ...
  -- Extract client IP if provided
  IF p_booking->>'client_ip' IS NOT NULL AND p_booking->>'client_ip' != '' THEN
    v_client_ip := (p_booking->>'client_ip')::INET;
  END IF;
...
-- 3. Lock down direct execution
REVOKE EXECUTE ON FUNCTION public.create_booking_atomic(jsonb) FROM public;
REVOKE EXECUTE ON FUNCTION public.create_booking_atomic(jsonb) FROM anon;
